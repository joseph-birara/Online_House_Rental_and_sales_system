...
const Application = ({ data }) => {
  ...
  const hadleClick = (app_id) => {
    ...
    const updatedAppplications = applications.filter(
      (AAA) => AAA._id !== app_id
    );
    ...
    axios
      .delete(
        ` https://house-rental.onrender.com/application/delete/${app_id}`,
        {
          headers: {
  ...
  return (
    ...
    >
        ...
        <div className=" flex justify-start gap-4">
          ...
          <div className="px-2">
            <p>
              ...
              <span>{data.checkin ? data.checkin : "Not specified"}</span>
            </p>
            <p>
              <span>Checkout date: </span>{" "}
              <span>{data.checkout ? data.checkout : "Not specified"}</span>
            </p>
            <p className="pt-2">
              <span>App Status: </span>{" "}
              <span
                className={` text-white mx-3 pb-1 px-2 rounded-xl outline  ${
                  data.status === "accepted" ? "bg-[green]" : ""
                } ${data.status === "completed" ? "bg-[#4ff23d]" : ""}  ${
                  data.status === "rejected" ? "bg-[red]" : ""
                }  ${data.status === "pending" ? "bg-[#dfdf0e]" : ""}`}
              >
                {data.appStatus}
              </span>
            </p>
      ...
      {data.appStatus && data.appStatus === "pending" && (
        <div className=" mx-auto">
          <button
            className="p-2 mx-2 text-lg rounded-lg  text-white bg-[red] block  ml-5"
